...
``--host 0.0.0.0``. Never bind to a public interface without
further hardening; ``AVIS_WEB_TOKEN`` is the only auth layer.

The CLI reads ``.env`` in the current working directory, then
expects::
...
def _load_dotenv(path: Path) -> None:
    ...
    format we accept is the simple subset already used elsewhere in
    the project: lines of ``KEY=VALUE``, ignore blank lines and
    ``#`` comments. Quoted values have surrounding quotes stripped.
    Existing environment variables take precedence; ``.env`` only
    fills gaps.
    """
    if not path.exists():
        return
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Avis web dashboard. Token-authenticated FastAPI app. "
            "Reads .env from the cwd for AVIS_WEB_TOKEN."
        ),
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable auto-reload (dev only -- not recommended on the "
            "Pi where it adds inotify overhead and conflicts with "
            "systemd)."
        ),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 2. Check the token before binding the socket -- fail with a
    #    clear message at startup instead of returning 500 on the
    #    first request.
    try:
    ...
    # 3. Show a startup banner so the operator can grab the URL.
    ...
    # 4. Build the app. The factory is cheap so tests can construct
    #    it freely.
    try:
        app = create_app()
    ...
    # 5. Run uvicorn. Pass the app object directly so the reloader
